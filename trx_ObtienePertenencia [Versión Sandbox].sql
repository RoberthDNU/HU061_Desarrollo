...
BEGIN
	...
	FROM   Colectivas GRUPO
	       INNER JOIN Colectivas CADENA
	            ON  CADENA.ID_ColectivaPadre = GRUPO.ID_Colectiva
	       INNER JOIN Colectivas SUCURSAL
	            ON  SUCURSAL.ID_ColectivaPadre = CADENA.ID_Colectiva
	       INNER JOIN Colectivas AFILIACION
	...
	WHERE  AFILIACION.ClaveColectiva = @Afiliacion
	       AND AFILIACION.ID_TipoColectiva = 1    
                ...
                FROM Colectivas col
                    INNER JOIN ColectivaMedioAcceso cMe
                        ON (col.ID_Colectiva = cMe.ID_Colectiva)
                    INNER JOIN MediosAcceso med
                        ON (cMe.ID_MA = med.ID_MA)
                WHERE med.ClaveMA = @MedioAcceso and med.ID_TipoMA=1
	...
	BEGIN
	    SELECT TOP 1 
	           ...
	           @ID_TipoCuenta          = ISNULL(c.ID_TipoCuenta, 0)
	    FROM   Cuentas c
	           INNER JOIN GruposCuenta gc
	                ON  c.ID_GrupoCuenta = gc.ID_GrupoCuenta
	           INNER JOIN TipoCuenta tc
	                ON  c.ID_TipoCuenta = tc.ID_TipoCuenta
	           INNER JOIN MediosAccesoCuenta mac
	                ON  mac.ID_Cuenta = c.ID_Cuenta
	           INNER JOIN MediosAcceso ma
	                ON  mac.ID_MA = ma.ID_MA
	           INNER JOIN TipoMedioAcceso tma
	                ON  tma.ID_TipoMA = ma.ID_TipoMA
	    WHERE  ma.ClaveMA = @MedioAcceso
	           AND tma.Clave = @TipoMedioAcceso
	    ORDER BY
	           TC.CodTipoCuentaISO ASC    
	...
	FROM   dbo.RelacionPadresColectivas PADRE
	       LEFT JOIN dbo.RelacionPadresColectivas ABUELO
	...
	BEGIN
	    SELECT @ID_PadreDeCuentahabiente = ISNULL(dbo.Colectivas.ID_Colectiva, 0)
	    FROM   dbo.Colectivas
	...
	BEGIN
	    SELECT @ID_AbueloDeCuentahabiente = ISNULL(dbo.Colectivas.ID_Colectiva, 0)
	    FROM   dbo.Colectivas
	    WHERE  ClaveColectiva = 'CADDNU'
	END 
	...
	IF @PlugInInstancia = 'isoPlugInEVERTEC' or @PlugInInstancia = 'isoPlugInEVERTEC_CL' OR  @PlugInInstancia = 'isoPlugInSERFINSA'
	BEGIN
	    SELECT @ID_Pertenencia = ISNULL(ID_Pertenencia, 0)
	    FROM   Pertenencia pe
	    ...
	    BEGIN
	        SET @ERROR = '[trx_ObtienePertenencia] -NO HAY RELACIONES DE PERTENENCIA ' 
	            ...
	            + ', ClaveAfiliacion =' + CONVERT(VARCHAR(20), @Afiliacion) 
	            --+ ', Beneficiario ='                              + CONVERT(VARCHAR(20), @Beneficiario)    
	            + ', CodigoProceso =' + CONVERT(VARCHAR(20), @ProcessingCode) 
	            + ', TipoMensaje =' + CONVERT(VARCHAR(20), @TipoMensaje) 
	            + ', ID_GrupoMA =' + CONVERT(VARCHAR(20), @ID_GrupoMA) 
	            -- + ', TipoCuentaISO ='                              + CONVERT(VARCHAR(20), @TipoCuenta)    
	            + ', ID_GrupoCuenta =' + CONVERT(VARCHAR(20), @ID_GrupoCuenta) 
	            -- + ', ID_ColectivaEmisor ='                             + CONVERT(VARCHAR(20), @ID_Emisor)       
	            + ', moneda =' + CONVERT(VARCHAR(20), @CodigoMoneda) 
	...
	BEGIN
	    ...
	    WHERE  --pe.ClaveAfiliacion = @Afiliacion
	           --AND pe.Beneficiario = @Beneficiario    
	           /*AND */ 
	           pe.CodigoProceso = @ProcessingCode
	           AND pe.TipoMensaje = @TipoMensaje
	           AND PE.ID_GrupoMA = @ID_GrupoMA 
	               --AND pe.TipoCuentaISO = SUBSTRING(@ProcessingCode,
	               --                              3, 2)
